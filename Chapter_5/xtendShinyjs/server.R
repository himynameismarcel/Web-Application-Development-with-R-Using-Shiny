...
function(input, output) {
  ...
  # the observe event for the extended javascript is pretty simple
  ...
  # when the map tab is selected, we disable year control
  # by means of shinyjs
  observe({
  ...
  # another simple trick is changing the formatting of text or tables;
  # we can do this very simply using the toggleClass() function, which 
  # adds and takes away a CSS class to/from a div.
  observe({
    toggleClass("theText", "redText", input$redText)
  })
  
  # reset the year slide
  # now, we use the observeEvent() to listen for the button push, and
  # reset() to reset the value of the named div:
  observeEvent(input$reset, {
    reset("yearPanel")
  })
  
  # lastly, we add an example for the oneevent function
  # (we're going to listen for the hover event, which refers to when 
  # the mouse pointer is above something)
  onevent("hover", "year",
           html("controllList",
